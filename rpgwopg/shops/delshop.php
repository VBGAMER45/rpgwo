...
include($root_path . "header.php");
	...
	If(verifysession($s))
	{
		...
		$Result = site_query("SELECT Username, ID FROM storepost", $dblink);
			if (!$Result) {
  			echo("<p>Error performing query on storepost: " . mysqli_error() . "</p>");
  			exit();
			}
		$good=0;
		...
		$sql = "Delete FROM storepost
			WHERE ID='$ID'";


  		if (@site_query($sql)) {
    			echo("<p>Your Shop has been deleted!</p>");
 		 }
		else {
    				echo("<p>Error deleting shop:" . mysqli_error() . "</p>");
  			}
...
<HTML>
   <HEAD>
	...
	<base href="http://www.projectxonline.net/rpgwo/">
     ...
     <TITLE>RPGWO-PG: Delete Shop </TITLE>
   </HEAD>
...
<!-- END MENU CODE -->
        ...
        <td width="520" height="25"  style="background-color: #0080FF">
          <p align="center"><font style="font-weight: bold">Delete Shop</font></p>
        </td>
      </tr>
      <tr>
...
<form action=shops/delshop.php method="POST">
  <center>
  ...
  <p><input type="submit" value="Delete Shop" name="B1"></p>
