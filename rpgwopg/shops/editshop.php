...
include($root_path . "header.php");
	...
	If(verifysession($s))
	{
		...
		$Result = site_query("SELECT Username, ID FROM storepost", $dblink);
			if (!$Result) {
  			echo("<p>Error performing query on guildpost: " . mysqli_error() . "</p>");
  			exit();
			}
		$good=0;
		...
		if($good==0)
		{
			print("You do not own that shop!");
			exit();
		}
		//######End of verify!
		...
		$sql = "Update storepost SET
  		...
  		if (@site_query($sql)) {
    			echo("<p>Your shop has been Updated!</p>");
 		 }
		else {
    				echo("<p>Error updating shop:" . mysqli_error() . "</p>");
  			}
...
<HTML>
   <HEAD>
	...
	<base href="http://www.projectxonline.net/rpgwo/">
     ...
     <TITLE>RPGWO-PG: Edit Shop  </TITLE>
   </HEAD>
...
<!-- END MENU CODE -->
        ...
        <td width="520" height="25"  style="background-color: #0080FF">
          <p align="center"><font style="font-weight: bold">Edit Shop</font></p>
        </td>
      </tr>
      <tr>
...
<form action=shops/editshop.php method="POST">
  <center>
  ...
  <p><input type="submit" value="Edit Shop" name="B1"></p>
