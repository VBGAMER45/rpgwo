...
function GetSingle($handle)
{
	$string[0] = fgetc($handle);
	$string[1] = fgetc($handle);
        $string[2] = fgetc($handle);
        $string[3] = fgetc($handle);
	return("Not Working yet");
}



$handle = fopen ("itemdef.dat", "r");
//while (!feof ($handle)) {
for ($g=0; $g <10; $g++)
{
   ...
   //Print out Animation
   $buffer = GetInt2($handle);
print "Animation0=" . $buffer;
 $PicSize=0;
 $buffer=$buffer+1;
 echo "<img src=\"itemlist/findItempic.php?PicNum=".$buffer."&PicSize=".$PicSize."\"></td><td>";
    ...
    for($i=1;$i<10; $i++)
   {
    ...
    print "<br>";	
    print "Animation$i=" . GetInt2($handle);
   }
    //Get class
