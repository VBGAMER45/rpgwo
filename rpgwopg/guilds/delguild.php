...
include($root_path . "header.php");
	...
	If(verifysession($s))
	{
		...
		if($good==0)
		{
			print("You do not own that guild!");
			exit();
		}
		//######End of verify!
		$ID=addslashes($ID);

		$sql = "Delete FROM guildpost
			WHERE ID='$ID'";


  		if (@site_query($sql)) {
    			echo("<p>Your Guild has been deleted!</p>");
 		 }
		else {
    				echo("<p>Error deleting guild:" . mysqli_error() . "</p>");
  			}
...
<HTML>
   <HEAD>
	...
	<base href="http://www.projectxonline.net/rpgwo/">
     ...
     <TITLE>RPGWO-PG: <? print($name); ?> - Delete Post </TITLE>
   </HEAD>
...
<form action=guilds/delguild.php method="POST">
  <center>
  ...
  <p><input type="submit" value="Delete Guild" name="B1"></p>
