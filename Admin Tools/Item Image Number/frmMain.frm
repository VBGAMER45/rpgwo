...
Begin VB.Form frmMain 
   Caption         =   "Items Number Finder RPGWO.net"
   ClientHeight    =   6810
   ClientLeft      =   45
   ClientTop       =   345
   ClientWidth     =   4830
...
'Test if directory cotains the bitmaps if not goes to nofile
    FileSource = App.Path
    Me.Picture = LoadPicture(FileSource & "\item0.bmp")
    For i = 1 To 99
        MaxPage = i
        Image1.Picture = LoadPicture(FileSource & "\item" & i & ".bmp")
    Next
    varImg = 0
    Page = 0
...
On Error GoTo nofile:
        ...
        Me.Picture = LoadPicture(FileSource & "\item" & (MaxPage - 1) & ".bmp")
        Page = MaxPage - 1
...
On Error GoTo nofile:
    If Page < MaxPage Then
        Page = Page + 1
        Me.Picture = LoadPicture(FileSource & "\item" & Page & ".bmp")
        lblPrev.Enabled = True
        Call ResetShape
    End If
Exit Sub
